...
def make_workbook() -> openpyxl.Workbook:
    ...
    Contient :
    - Feuille Config : 3 étudiants (Alice, Bob anonyme, Eve avec date de départ)
    - Feuille S1 : séance 1 avec les 3 étudiants (présences variées, scores)
    ...
    config.cell(row=4, column=2, value="Anonyme")
    config.cell(row=4, column=3, value="Pseudo")
    config.cell(row=4, column=4, value="Date de départ")
    # Étudiants (ligne 5+)
    ...
    config.cell(row=6, column=2, value="oui")
    config.cell(row=6, column=3, value="BobM")
    config.cell(row=7, column=1, value="Leclerc Eve")
    config.cell(row=7, column=2, value="non")
    config.cell(row=7, column=4, value="2026-02-15")
    ...
    s1.cell(row=7, column=2, value="R15")
